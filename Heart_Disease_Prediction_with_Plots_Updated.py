...
model.compile(optimizer='adam',
                loss=tf.keras.losses.BinaryCrossentropy(from_logits=True),
                metrics=['accuracy'])
history = model.fit(train_features, train_target, epochs=5, batch_size=BATCH_SIZE, validation_data=(val_features, val_target), callbacks=[ES, LR, TB, MC, ROP,])
print(f'\n{history}')
...
model.save('my_heart_disease_model.h5')

# Save weights and biases
model.save_weights('my_heart_disease_model_weights.h5')

# Load the entire model
loaded_model = load_model('my_heart_disease_model.h5')

# Load weights and biases
loaded_model.load_weights('my_heart_disease_model.h5')
...
prediction1 = loaded_model.predict(sample_features1)
...
# Predict using the model
prediction2 = loaded_model.predict(sample_features2)
...
# Predict using the model
prediction3 = loaded_model.predict(sample_features3)
...
print(f'Patient 2 has chances of heart attack' if max(probabilities2.numpy()) > 0.5 else 'Patient 2 is safe')
...
print(f'Patient 3 has chances of heart attack' if max(probabilities3.numpy()) > 0.5 else 'Patient 3 is safe')
